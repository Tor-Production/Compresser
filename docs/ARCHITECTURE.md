...
|---|---|---|
| `cim-core` | The codec: bit I/O, header, block geometry, encode, decode. | No image-format dependencies. `#![forbid(unsafe_code)]`. Must stay `wasm32`-clean and C-ABI-wrappable. |
| `cim-imageio` | PNG and WebP bridge: `DynamicImage` to and from `RawImage`. | The only crate that depends on `image`. Exists so the CLI and the benchmark share the conversion without one depending on the other. |
| `cim-cli` | `encode` / `decode` / `info` commands. | All file I/O goes through `cim-imageio`. |
| `cim-bench` | Compression-ratio table against PNG and WebP across block sizes, plus the `gen-samples` corpus generator. | Verifies losslessness on real images as a side effect. |

`cim-core` deliberately does not depend on `image`. Pulling a PNG decoder into the codec would
...
error.rs    CimError — one typed error per validation rule in FORMAT.md §8.
...
analysis.rs Walks a bitstream without reconstructing pixels. Backs `cim info`.
```
...
2. **No panics on untrusted input.** Every malformed byte sequence yields `Err(CimError)`.
...
`BitReader` rather than indexing the buffer itself. Measure with `cargo bench -p cim-core` before
