...
Measured results from `cim-lab`. **Nothing here is part of the format** — see
[FORMAT.md](FORMAT.md) for what a `.cim` file actually is. This document exists so the roadmap is
...
| `filter+zigzag+cim[8x8]+deflate` | 65.6% | 21 MiB/s | 84 MiB/s |
| `filter+zigzag+cim[8x8]+huffman` | 66.4% | 57 MiB/s | 40 MiB/s |
| `raw+deflate` | 66.9% | 28 MiB/s | 184 MiB/s |
| `quadtree+deflate` | 71.9% | 19 MiB/s | 119 MiB/s |
| `quadtree` | 73.8% | 32 MiB/s | 169 MiB/s |
| `filter+zigzag+cim[8x8]` | 74.5% | 75 MiB/s | 106 MiB/s |
| `cim[8x8]+deflate` | 75.0% | 36 MiB/s | 129 MiB/s |
| `cim[8x8]` | 77.8% | 195 MiB/s | 209 MiB/s |
| `raw+huffman` | 93.7% | 193 MiB/s | 58 MiB/s |
| `raw+lzw` | 95.1% | 25 MiB/s | 86 MiB/s |
| `cim[whole]` | 100.0% | 194 MiB/s | 186 MiB/s |
| `filter+cim[8x8]` (no zigzag) | 102.3% | 74 MiB/s | 120 MiB/s |

The throughput columns predate the refilling bit reader (finding 10): every pipeline that decodes
through `cim-core`'s `BitReader` is faster than shown. Sizes are unaffected.
...
| `filter+zigzag+cim[8x8]+deflate` | 51.6% | 25 MiB/s | 101 MiB/s |
| `filter+zigzag+cim[8x8]+huffman` | 53.0% | 65 MiB/s | 52 MiB/s |
| `filter+huffman` | 53.6% | 76 MiB/s | 66 MiB/s |
| `raw+deflate` | 56.0% | 33 MiB/s | 241 MiB/s |
| `quadtree+deflate` | 56.6% | 25 MiB/s | 153 MiB/s |
| `cim[8x8]+deflate` | 58.2% | 45 MiB/s | 176 MiB/s |
| `quadtree` | 60.9% | 42 MiB/s | 222 MiB/s |
| `cim[8x8]+huffman` | 62.3% | 133 MiB/s | 65 MiB/s |
| `cim[8x8]` | 65.7% | 225 MiB/s | 261 MiB/s |
| `raw+lzw` | 80.9% | 30 MiB/s | 107 MiB/s |
| `filter+cim[8x8]` | 84.9% | 78 MiB/s | 136 MiB/s |
| `cim[whole]` | 90.1% | 212 MiB/s | 229 MiB/s |
...
On the synthetic corpus alone, `cim[8x8]+deflate` came first at 21.2%, beating `filter+deflate` at
...
**Any future claim about Cimilarity must be checked on photographs before it is believed.**
...
`filter+cim[8x8]` produces a file **larger than the raw samples** — 102.3%.
...
|---|---:|---:|
| `filter+cim[8x8]` | 102.3% | 84.9% |
| `filter+zigzag+cim[8x8]` | **74.5%** | **61.8%** |

A one-line bijection is worth 28 percentage points. It also turns the composition from actively
harmful into the best Cimilarity-family result once an entropy stage is added: 65.6% on photographs,
against 75.0% for `cim[8x8]+deflate`.

### 4. Adaptive block size is a real but modest win

`quadtree` beats `cim[8x8]` by 4 points on photographs (73.8% against 77.8%) and 4.8 on the full
...
Notably, `quadtree+deflate` (56.6%) is *worse* than `cim[8x8]+deflate` on the synthetic corpus at
...
### 6. Speed is where Cimilarity is strong

`cim[8x8]` decodes at 261 MiB/s, the fastest pipeline that compresses at all, and encodes at
...
Prior art names Cimilarity's core exactly: storing a block minimum and packing the offsets at a shared bit
width is **Frame of Reference** coding, standard in column stores and inverted indexes. The known
weakness there is the same one Cimilarity has — a single extreme value sets the width for the whole block
