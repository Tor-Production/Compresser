...
//!
//! **Nothing here is part of the CIM format.** `docs/FORMAT.md` describes what a `.cim` file is;
...
pub struct Cim {
    ...
    pub filter: cim_core::FilterChoice,
    /// Which block coder the format itself is asked to use.
    pub coder: cim_core::CoderChoice,
}
...
impl Codec for Cim {
    fn name(&self) -> String {
        ...
        let f = match self.filter {
            cim_core::FilterChoice::Off => "",
            cim_core::FilterChoice::On => ",pred",
            cim_core::FilterChoice::Auto => ",auto",
        };
        let c = match self.coder {
            cim_core::CoderChoice::Fixed => "",
            cim_core::CoderChoice::Rice => ",rice",
            cim_core::CoderChoice::Auto => ",bestcoder",
        };
        format!("cim[{block}{f}{c}]{}", self.entropy.suffix())
    }

    fn encode(&self, img: &RawImage) -> Result<Vec<u8>> {
        let opts = cim_core::EncodeOptions {
            block_size: self.block.map(|n| (n, n)),
            filter: self.filter,
            coder: self.coder,
            ..Default::default()
        };
        let raw = cim_core::encode(img, &opts).map_err(|e| anyhow::anyhow!(e))?;
        self.entropy.pack(&raw)
    }

    fn decode(&self, bytes: &[u8]) -> Result<RawImage> {
        let raw = self.entropy.unpack(bytes)?;
        cim_core::decode(&raw).map_err(|e| anyhow::anyhow!(e))
    }
}
...
/// this is essentially PNG minus the chunk framing, and is the number CIM has to beat.
pub struct Filtered {
    pub entropy: Entropy,
}
...
/// Prediction first, then CIM's range packing over the residuals.
///
...
impl Codec for FilteredCim {
    fn name(&self) -> String {
        ...
        format!("filter{zz}+cim[{block}]{}", self.entropy.suffix())
    }
    ...
    fn encode(&self, img: &RawImage) -> Result<Vec<u8>> {
        ...
        let opts = cim_core::EncodeOptions {
            block_size: self.block.map(|n| (n, n)),
            filter: cim_core::FilterChoice::Off,
            coder: cim_core::CoderChoice::Fixed,
            ..Default::default()
        };
        let packed = cim_core::encode(&residuals, &opts).map_err(|e| anyhow::anyhow!(e))?;
    ...
    fn decode(&self, bytes: &[u8]) -> Result<RawImage> {
        ...
        let residuals = cim_core::decode(&body[rows..]).map_err(|e| anyhow::anyhow!(e))?;
        if self.zigzag {
...
/// Range-shaped prediction, then CIM. The variant study behind roadmap item 1.
...
impl Codec for Predicted {
    fn name(&self) -> String {
        ...
        format!("pred[{h},{sc}]+cim[{block}]{}", self.entropy.suffix())
    }
    ...
    fn decode(&self, bytes: &[u8]) -> Result<RawImage> {
        ...
        let residuals = cim_core::decode(&body[n..]).map_err(|e| anyhow::anyhow!(e))?;
...
pub fn all_codecs() -> Vec<Box<dyn Codec>> {
    ...
    use cim_core::{CoderChoice, FilterChoice};
