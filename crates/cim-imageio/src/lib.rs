//! The bridge between the `image` crate and [`cim_core::RawImage`].
//!
//! `cim-core` deliberately knows nothing about PNG or WebP, so every binary that reads or writes
//! real image files needs this conversion. It lives here rather than in `cim-cli` so the CLI and
...
pub struct Loaded {
    pub image: RawImage,
    /// True when the source had more than 8 bits per sample and had to be reduced, so a CIM
...
pub fn from_dynamic(img: DynamicImage) -> Result<Loaded> {
    ...
    let (channels, data, narrowed) = match img {
        // The four layouts CIM v1 stores natively.
...
mod tests {
    ...
    fn recognizes_supported_extensions() {
        ...
        assert!(!is_supported_image(Path::new("a.cim")));
