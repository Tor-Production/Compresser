...
use cim_core::{
    analyze, decode, encode, ChannelOptions, CimError, CoderChoice, EncodeOptions, FilterChoice,
    RawImage,
};
...
fn empty_and_tiny_inputs() {
    for len in 0..BODY_AT {
        ...
        assert!(matches!(
            ...
            CimError::BadMagic | CimError::HeaderTooShort { .. }
        ));
    }
}
...
fn bad_magic() {
    ...
    bytes[1] = b'X';
    assert_eq!(decode(&bytes).unwrap_err(), CimError::BadMagic);
}

/// Neither earlier generation of the magic may be mistaken for a current file: `BRP1` from
/// version 1, and `BRP` from versions 2 to 4, written before the format was renamed. Files
/// with those magics exist on developers' disks; they are not Cimilarity files. See ADR 0008.
#[test]
fn the_previous_format_generations_are_rejected() {
    for magic in [b"BRP1", b"BRP"] {
        let mut bytes = sample_file();
...
fn absurd_dimensions_do_not_allocate() {
    ...
    assert_eq!(decode(&bytes).unwrap_err(), CimError::DimensionOverflow);
}
...
fn large_declared_geometry_with_empty_body() {
    ...
    assert!(matches!(
        decode(&bytes).unwrap_err(),
        CimError::UnexpectedEof | CimError::InvalidWidthCode(_)
    ));
    assert!(matches!(
...
fn alias_validation() {
    ...
    assert_eq!(
        decode(&bytes).unwrap_err(),
        CimError::AliasTargetBitsSet(0b0001_0000)
    );

    // Channel 0 can never alias.
    let mut bytes = original.clone();
    bytes[MODES_AT] = 0b00_00_00_10;
    assert_eq!(decode(&bytes).unwrap_err(), CimError::AliasOnFirstChannel);
}
...
fn arbitrary_filter_modes() {
    ...
    for mode in 0..=255u8 {
        ...
        if mode > 1 {
            assert!(matches!(
                decoded.unwrap_err(),
                CimError::UnsupportedFilterMode(_)
            ));
        }
    }
}
...
fn arbitrary_block_coders() {
    ...
    for coder in 0..=255u8 {
        ...
        if coder > 1 {
            assert!(matches!(
                decoded.unwrap_err(),
                CimError::UnsupportedBlockCoder(_)
            ));
        }
    }
}
