...
/// ASCII `CIM` followed by 0x1A. The trailing byte is PNG's trick: it stops `type` on DOS-derived
...
pub const MAGIC: [u8; 4] = [b'C', b'I', b'M', 0x1A];
...
impl Header {
    ...
    pub fn parse(bytes: &[u8]) -> Result<(Self, usize)> {
        if bytes.len() < HEADER_BASE_SIZE {
            return Err(CimError::HeaderTooShort {
                got: bytes.len(),
                need: HEADER_BASE_SIZE,
            });
        }
        if bytes[0..4] != MAGIC {
            return Err(CimError::BadMagic);
        }
        let version = bytes[4];
        if version != VERSION {
            return Err(CimError::UnsupportedVersion {
                found: version,
                expected: VERSION,
            });
        }
        let flags = bytes[5];
        if flags != 0 {
            return Err(CimError::ReservedFlagsSet(flags));
        }
        ...
        if !matches!(channels, 1..=4) {
            return Err(CimError::InvalidChannelCount(channels));
        }
        if bit_depth != BIT_DEPTH {
            return Err(CimError::UnsupportedBitDepth(bit_depth));
        }
        for (value, what) in [
            (width, "width"),
            (height, "height"),
            ...
            if value == 0 {
                return Err(CimError::ZeroDimension { what });
            }
        }
        ...
        if filter_mode > FILTER_MODE_ADAPTIVE {
            return Err(CimError::UnsupportedFilterMode(filter_mode));
        }

        let block_coder = bytes[25];
        if block_coder > BLOCK_CODER_RICE {
            return Err(CimError::UnsupportedBlockCoder(block_coder));
        }
        ...
        if filter_mode == FILTER_MODE_ADAPTIVE && plan.coded_count() == 0 {
            return Err(CimError::FilterWithoutCodedChannels);
        }
...
mod tests {
    ...
    fn field_offsets_match_the_spec() {
        ...
        assert_eq!(&bytes[0..4], &[b'C', b'I', b'M', 0x1A]);
    ...
    fn rejects_bad_magic_and_version() {
        ...
        bytes[3] = b'1'; // "CIM1" -- close enough to look right, still not ours
        assert_eq!(Header::parse(&bytes).unwrap_err(), CimError::BadMagic);

        let mut bytes = encoded(&sample());
        bytes[4] = 3;
        assert_eq!(
            Header::parse(&bytes).unwrap_err(),
            CimError::UnsupportedVersion {
                found: 3,
                expected: 4
            }
        );
    ...
    fn rejects_bad_channels_and_depth() {
        ...
        assert_eq!(
            Header::parse(&bytes).unwrap_err(),
            CimError::InvalidChannelCount(0)
        );
        ...
        assert_eq!(
            Header::parse(&bytes).unwrap_err(),
            CimError::UnsupportedBitDepth(16)
        );
    }
    ...
    fn rejects_an_unknown_filter_mode() {
        for mode in 2..=255u8 {
            ...
            assert_eq!(
                Header::parse(&bytes).unwrap_err(),
                CimError::UnsupportedFilterMode(mode)
            );
        }
    }
    ...
    fn rejects_an_unknown_block_coder() {
        for coder in 2..=255u8 {
            ...
            assert_eq!(
                Header::parse(&bytes).unwrap_err(),
                CimError::UnsupportedBlockCoder(coder)
            );
        }
    }
    ...
    fn rejects_prediction_with_nothing_to_predict() {
        ...
        assert_eq!(
            ...
            CimError::FilterWithoutCodedChannels
        );
    }
