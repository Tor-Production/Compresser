...
/// Encodes an image into a CIM v4 bitstream.
...
fn encode_with_filter(img: &RawImage, opts: &EncodeOptions, filter: bool) -> Result<Vec<u8>> {
    ...
    if block_w == 0 {
        return Err(CimError::ZeroDimension { what: "block_w" });
    }
    if block_h == 0 {
        return Err(CimError::ZeroDimension { what: "block_h" });
    }
...
mod tests {
    ...
    fn rejects_zero_block_size() {
        ...
        assert_eq!(
            ...
            CimError::ZeroDimension { what: "block_w" }
        );
    }
