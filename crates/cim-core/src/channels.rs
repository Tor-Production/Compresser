...
impl ChannelPlan {
    ...
    pub fn parse(bytes: &[u8], channels: u8) -> Result<(Self, usize)> {
        ...
        let modes_byte = *bytes.first().ok_or(CimError::HeaderTooShort {
            got: bytes.len(),
            need: 1,
        })?;
        ...
        if used_bits < 8 && (modes_byte >> used_bits) != 0 {
            return Err(CimError::ChannelModeBitsSet(modes_byte));
        }
        ...
        for (c, code) in codes.iter_mut().enumerate().take(n) {
            *code = (modes_byte >> (2 * c)) & 0b11;
            match *code {
                ...
                MODE_ALIAS => {
                    if c == 0 {
                        return Err(CimError::AliasOnFirstChannel);
                    }
                    alias_count += 1;
                }
                _ => return Err(CimError::ReservedChannelMode(MODE_RESERVED)),
            }
        }
        ...
        let alias_byte = if alias_count > 0 {
            let b = *bytes.get(consumed).ok_or(CimError::HeaderTooShort {
                got: bytes.len(),
                need: consumed + 1,
            })?;
            let used = 2 * alias_count;
            if used < 8 && (b >> used) != 0 {
                return Err(CimError::AliasTargetBitsSet(b));
            }
            consumed += 1;
            b
        } else {
            ...
            match codes[c] {
                ...
                MODE_ALIAS => {
                    ...
                    if usize::from(target) >= c {
                        return Err(CimError::InvalidAliasTarget {
                            channel: c as u8,
                            target,
                        });
                    }
                    ...
                    if codes[usize::from(target)] != MODE_CODED {
                        return Err(CimError::AliasTargetNotCoded {
                            channel: c as u8,
                            target,
                        });
                    }
...
mod tests {
    ...
    fn rejects_a_reserved_mode() {
        ...
        assert_eq!(
            ...
            CimError::ReservedChannelMode(3)
        );
    }
    ...
    fn rejects_mode_bits_for_absent_channels() {
        ...
        assert_eq!(
            ...
            CimError::ChannelModeBitsSet(0b0100)
        );
    }
