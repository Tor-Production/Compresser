...
/// PNG or WebP into one of these is the caller's job, so that `cim-core` stays dependency-free.
...
impl RawImage {
    ...
    pub fn new(width: u32, height: u32, channels: u8, data: Vec<u8>) -> Result<Self> {
        ...
        if width == 0 {
            return Err(CimError::ZeroDimension { what: "width" });
        }
        if height == 0 {
            return Err(CimError::ZeroDimension { what: "height" });
        }
        let need = required_len(width, height, channels)?;
        if data.len() != need {
            return Err(CimError::BufferLengthMismatch {
                got: data.len(),
                need,
                width,
                height,
...
mod tests {
    ...
    fn rejects_bad_geometry() {
        assert_eq!(
            ...
            CimError::ZeroDimension { what: "width" }
        );
        assert_eq!(
            RawImage::new(2, 0, 3, vec![]).unwrap_err(),
            CimError::ZeroDimension { what: "height" }
        );
        assert_eq!(
            RawImage::new(2, 2, 5, vec![0; 20]).unwrap_err(),
            CimError::InvalidChannelCount(5)
        );
    }
