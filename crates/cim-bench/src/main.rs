//! Compression-ratio benchmark: CIM against PNG and lossless WebP, across block sizes.
//!
...
//!
//! Usage: `cargo run -p cim-bench --release -- samples/`

use anyhow::{bail, Context, Result};
use cim_core::{decode, encode, CoderChoice, EncodeOptions, FilterChoice, RawImage};
...
fn measure(path: &Path) -> Result<Row> {
    let loaded = cim_imageio::load(path)?;
    let img = loaded.image;
    let raw = img.data().len();

    let png = cim_imageio::encode_png(&img)?.len();
    let webp = cim_imageio::encode_webp_lossless(&img)?.len();

    let mut cim = Vec::with_capacity(BLOCK_SIZES.len());
...
fn print_table(rows: &[Row]) {
    ...
    println!("\nPercentages are of raw sample bytes; lower is smaller. All CIM entries were");
    ...
    let totals_whole: usize = rows.iter().map(|r| r.cim[0]).sum();
    ...
    println!(
        "  cim whole image   {:>10}   {:>5.1}%",
        human(totals_whole),
        pct(totals_whole, totals_raw)
    );
    println!(
        "  cim best block    {:>10}   {:>5.1}%",
        human(totals_best),
